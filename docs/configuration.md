...
| ------------------ | -------------------------------- | ---------------------------- | -------------------------------------------------------------------------------- |
| `apiBaseUrl`       | `https://twexts.sdisk.us/api/v0` | `config.yml` / `config.yaml` | API base URL the UI calls; must be a full `http`/`https` URL                     |
...
- An `apiBaseUrl` value is accepted only if it parses as `http` or `https`; anything else is logged and skipped in favor of the next candidate.
...
- `server.js` serves hashed assets with immutable cache headers (`public, max-age=31536000`) and `index.html` with `no-cache`.
